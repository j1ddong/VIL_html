...
class ChatStat {
  private readonly chatCount = new Map<ChatRoomId | "All", Map<SenderId, number>>([
    ["All", new Map()],
  ]);
  ...
  private compareTalkInRoom(chatRoomId: ChatRoomId, senderId: SenderId): void {
    if (
      !this.mostTalkativeInRoom[chatRoomId] ||
  ...
  private compareBusyPerson(senderId: SenderId): void {
    ...
    if (
      this.getRoomListForPerson(senderId).size > this.getRoomListForPerson(this.mostBusyPerson).size
    ) {
      this.mostBusyPerson = senderId;
    }
  }
...
function getChattingStat(input: typeof chatHistory) {
...
console.log(getChattingStat(chatHistory));
